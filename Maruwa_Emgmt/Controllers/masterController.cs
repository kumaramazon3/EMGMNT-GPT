...
namespace Maruwa_Emgmt.Controllers
{
    public class masterController : Controller
    {
        ...
        {
            // ReasonID is generated by dbo.usp_ReasonTypeMaster_Save while adding a new Reason.
            // It is posted only during edit, so it must not trigger validation for Add Reason.
            ModelState.Remove(nameof(ReasonMasterVm.ReasonID));
        ...
        private static byte[] CreateReasonCsv(IEnumerable<ReasonMasterVm> reasons)
        {
            ...
            sb.AppendLine("ReasonType,ReasonDescription,Created By,Created On,Edited By,Edited On,isActive");
            foreach (var d in reasons)
            {
                string Csv(string? value) => $"\"{(value ?? string.Empty).Replace("\"", "\"\"")}\"";
                sb.AppendLine(string.Join(',', Csv(d.ReasonType), Csv(d.ReasonDescription), Csv(d.CreatedBy), Csv(d.CreatedOn?.ToString("yyyy-MM-dd HH:mm")), Csv(d.EditedBy), Csv(d.EditedOn?.ToString("yyyy-MM-dd HH:mm")), Csv(d.isActive ? "Active" : "Inactive")));
            }
        ...
        private static byte[] CreateReasonPdf(IEnumerable<ReasonMasterVm> reasons)
        {
            ...
            string[] headers = ["ReasonType", "ReasonDescription", "Created By", "Created On", "Edited By", "Edited On", "Status"];
            foreach (var h in headers) table.AddCell(new Phrase(h));
            foreach (var d in reasons)
            {
                table.AddCell(d.ReasonType); table.AddCell(d.ReasonDescription); table.AddCell(d.CreatedBy ?? ""); table.AddCell(d.CreatedOn?.ToString("yyyy-MM-dd") ?? ""); table.AddCell(d.EditedBy ?? ""); table.AddCell(d.EditedOn?.ToString("yyyy-MM-dd") ?? ""); table.AddCell(d.isActive ? "Active" : "Inactive");
            }
            doc.Add(table);
            doc.Close();
            return ms.ToArray();
        ...
        private static string BuildReasonSheetXml(IEnumerable<ReasonMasterVm> reasons)
        {
            ...
            string[] headers = ["ReasonType", "ReasonDescription", "Created By", "Created On", "Edited By", "Edited On", "isActive"];
            int rowIndex = 1;
            rows.Append(BuildXlsxRow(rowIndex++, headers));
            foreach (var d in reasons)
            {
                rows.Append(BuildXlsxRow(rowIndex++, [d.ReasonType, d.ReasonDescription, d.CreatedBy ?? "", d.CreatedOn?.ToString("yyyy-MM-dd HH:mm") ?? "", d.EditedBy ?? "", d.EditedOn?.ToString("yyyy-MM-dd HH:mm") ?? "", d.isActive ? "Active" : "Inactive"]));
            }
