...
async function loadLeaveTypes() {
    try {
        ...
        showFormMessage('Error loading LeaveType data', false);
    }
}
...
function renderTable(data) {
    ...
    data.forEach(item => {
        ...
        const seqLeaveID = item.seqLeaveID || item.SeqLeaveID || 0;

        rows += `<tr>
            <td><i class="bi bi-pencil-square text-primary" style="cursor:pointer" onclick="editLeaveType(${seqLeaveID}, ${activeArg})"></i></td>
            <td><i class="bi bi-trash text-danger" style="cursor:pointer" onclick="confirmDeleteLeaveType(${seqLeaveID})"></i></td>
...
async function editLeaveType(seqLeaveID, activeStatusFromRow) {
    const response = await $.get('/master/GetLeaveType', { id: seqLeaveID });
    if (!response.success) {
    ...
    $('#seqLeaveID').val(d.seqLeaveID || d.SeqLeaveID || seqLeaveID);
    $('#leaveID').val(d.leaveID);
...
function confirmDeleteLeaveType(seqLeaveID) {
    deleteId = seqLeaveID;
...
async function exportLeaveType(format) {
    const response = await fetch('/master/ExportLeaveTypes?format=' + format, {
        method: 'POST',
    ...
    a.href = url;
    a.download = `LeaveTypeMaster.${format}`;
